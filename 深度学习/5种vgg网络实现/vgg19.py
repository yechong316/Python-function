...
def vgg11(x, n_class):
    ...
    layers = {
        ...
        'wc1_2':tf.get_variable('wc1_2', [3, 3, 64, 64], tf.float32),

        # conv-2
        'wc2_1': tf.get_variable('wc2_1', [3, 3, 64, 128], tf.float32),
        'wc2_2': tf.get_variable('wc2_2', [3, 3, 128, 128], tf.float32),

        # conv-3
        'wc3_1': tf.get_variable('wc3_1', [3, 3, 128, 256], tf.float32),
        'wc3_2': tf.get_variable('wc3_2', [3, 3, 256, 256], tf.float32),
        'wc3_3': tf.get_variable('wc3_3', [3, 3, 256, 256], tf.float32),
        'wc3_4': tf.get_variable('wc3_4', [3, 3, 256, 256], tf.float32),

        # conv-4
        'wc4_1': tf.get_variable('wc4_1', [3, 3, 256, 512], tf.float32),
        'wc4_2': tf.get_variable('wc4_2', [3, 3, 512, 512], tf.float32),
        'wc4_3': tf.get_variable('wc4_3', [3, 3, 512, 512], tf.float32),
        'wc4_4': tf.get_variable('wc4_4', [3, 3, 512, 512], tf.float32),

        # conv-5
        'wc5_1': tf.get_variable('wc5_1', [3, 3, 512, 512], tf.float32),
        'wc5_2': tf.get_variable('wc5_2', [3, 3, 512, 512], tf.float32),
        'wc5_3': tf.get_variable('wc5_3', [3, 3, 512, 512], tf.float32),
        'wc5_4': tf.get_variable('wc5_4', [3, 3, 512, 512], tf.float32),
        ...
        'bc1_2': tf.get_variable('bc1_2', [64], tf.float32),

        # conv-2
        'bc2_1': tf.get_variable('bc2_1', [128], tf.float32),
        'bc2_2': tf.get_variable('bc2_2', [128], tf.float32),

        # conv-3
        'bc3_1': tf.get_variable('bc3_1', [256], tf.float32),
        'bc3_2': tf.get_variable('bc3_2', [256], tf.float32),
        'bc3_3': tf.get_variable('bc3_3', [256], tf.float32),
        'bc3_4': tf.get_variable('bc3_4', [256], tf.float32),

        # conv-4
        'bc4_1': tf.get_variable('bc4_1', [512], tf.float32),
        'bc4_2': tf.get_variable('bc4_2', [512], tf.float32),
        'bc4_3': tf.get_variable('bc4_3', [512], tf.float32),
        'bc4_4': tf.get_variable('bc4_4', [512], tf.float32),

        # conv-5
        'bc5_1': tf.get_variable('bc5_1', [512], tf.float32),
        'bc5_2': tf.get_variable('bc5_2', [512], tf.float32),
        'bc5_3': tf.get_variable('bc5_3', [512], tf.float32),
        'bc5_4': tf.get_variable('bc5_4', [512], tf.float32),
    ...
    net = tf.nn.conv2d(input=net, filter=layers['wc1_2'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc1_2']))
    ...
    net = tf.nn.conv2d(input=net, filter=layers['wc2_2'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc2_2']))
    net = tf.nn.max_pool(net, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    net = tf.nn.conv2d(input=net, filter=layers['wc3_3'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc3_3']))
    net = tf.nn.conv2d(input=net, filter=layers['wc3_4'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc3_4']))
    net = tf.nn.max_pool(net, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    net = tf.nn.conv2d(input=net, filter=layers['wc4_3'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc4_3']))
    net = tf.nn.conv2d(input=net, filter=layers['wc4_4'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc4_4']))
    net = tf.nn.max_pool(net, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    net = tf.nn.conv2d(input=net, filter=layers['wc5_3'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc5_3']))
    net = tf.nn.conv2d(input=net, filter=layers['wc5_4'], strides=[1, 1, 1, 1], padding='SAME')
    net = tf.nn.relu(tf.nn.bias_add(net, layers['bc5_4']))
    net = tf.nn.max_pool(net, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
