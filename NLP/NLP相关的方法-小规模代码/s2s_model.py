...
class S2SModel(object):
    def __init__(self,
        ...
        softmax_loss_function = None # 不调用采样函数
        ...
        if num_samples > 0 and num_samples < self.target_vocab_size:
            ...
            w_t = tf.get_variable(
                "proj_w",
                [self.target_vocab_size, size], # （6865， 512）
                dtype=dtype
            )
            w = tf.transpose(w_t)
            b = tf.get_variable(
            ...
            def sampled_loss(labels, logits):
                ...
                local_w_t = tf.cast(w_t, tf.float32) # 映射的 w
                ...
                return tf.cast(
                    tf.nn.sampled_softmax_loss(
                        ...
                        inputs=local_inputs,     #预测出来的值，每次一个
                        num_sampled=num_samples, #采样512个词作为损失函数来训练输出值
                        num_classes=self.target_vocab_size #原始字典长度 6865
                    ),
        ...
        # #######################################################
        # 这是我们最核心的seq2seq_f函数体，前面都是预处理
        # #######################################################
        def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
            '''只执行seq2seq，不执行attention'''
            # Encoder.先将cell进行deepcopy，因为seq2seq模型是两个相同的模型，但是模型参数不共享，所以encoder和decoder要使用两个不同的RnnCell
            # 深拷贝，完完全全复制一份新的内容，在新的内存地址储存，seq2seq完全有两个
            tmp_cell = copy.deepcopy(cell) # tmp_cell = 3
            ...
            return tf.contrib.legacy_seq2seq.embedding_attention_seq2seq(
                encoder_inputs,# tensor of input seq 20
                decoder_inputs,# tensor of decoder seq 30
                tmp_cell,#自定义的cell,可以是GRU/LSTM, 设置multilayer等
                num_encoder_symbols=source_vocab_size,# 编码阶段，词典大小 40000
                num_decoder_symbols=target_vocab_size,# 解码阶段，目标词典大小 40000
                embedding_size=size,# embedding 维度 512
                # num_head = 1, # 全连接层使用1个 attention的头，数值越多越准确
                output_projection=output_projection,# 不设定的话输出维数可能很大(取决于词表大小)，设定的话投影到一个低维向量
                feed_previous=do_decode,# 是否执行的EOS。bool
                dtype=dtype
            )
...
# 跟language model类似，targets变量是decoder inputs平移一个单位的结果，
    ...
    def step(
        ...
        bucket_id,
        forward_only
    ): # encoder_size = 5, decoder_size = 15
        ...
        for i in range(encoder_size): # 10
            input_feed[self.encoder_inputs[i].name] = encoder_inputs[i] # 找变量的name
        ...
        input_feed[last_target] = np.zeros([self.batch_size], dtype=np.int32) # 最后一个设置为0

        if not forward_only: # 训练
            output_feed = [
                self.updates[bucket_id], # 更新后的梯度
                self.gradient_norms[bucket_id], # 梯度归一化
                self.losses[bucket_id] # 函数的损失值
            ]
            output_feed.append(self.outputs[bucket_id][i]) # 不同桶的都装进来
        else:
            output_feed = [self.losses[bucket_id]] # 当前的损失
            for i in range(decoder_size):
                output_feed.append(self.outputs[bucket_id][i])
        # 下面总共step总共输出4个东西，分别是优化器属性，阶段梯度，损失，各个隐藏层状态
        # <class 'list'>: [None, 0.29843816, loss=6.973443, output:<class 'tuple'>: (32, 512)]
    ...
    # dbs是字典
    def get_batch_data(self, bucket_dbs, bucket_id): # bucket的类属性，4个桶， bucket_id是桶的id号
        data = []
        data_in = []

        # bucket_id == 我们输入的语言 = 1
        bucket_db = bucket_dbs[bucket_id] # 取出要提取数据的桶
        # 随机抽取一个batch的对话框，512 * （ask + answer）
        for _ in range(self.batch_size): # 一个batch 32

            ask, answer = bucket_db.random() # ask='明蒂，我是瑞秋'，answer='我很好，我今天和巴瑞见面了'
